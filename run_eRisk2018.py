...
from model_eRisk2018 import MyNetwork
...
def train(args, model, optimizer, train_loader, val_loader, templates_torch, logger, test_loader=None):
    val_f1 = 0
    Train_Loss = []
    Val_Loss = []
    Val_F1 = []
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            for idx, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
                ...
                if idx == 0:
                    total_pred = logits
                    total_target = train_tags
                else:
                    total_pred = torch.cat((total_pred, logits), dim=0)
                    total_target = torch.cat((total_target, train_tags), dim=0)
        ...
        with torch.no_grad():
            for idx, batch in tqdm(enumerate(test_loader), total=len(test_loader)):
                ...
                logits, ls = model(test_data, test_tags, templates_torch[0])
                if idx == 0:
                    total_pred = logits
        ...
        F1 = f1_score(y_true=target, y_pred=pred)
        AUC = roc_auc_score(y_true=target, y_score=score)
        print("-----------------test_F1:{} , test_AUC:{}-----------------".format(F1, AUC))
        # print(classification_report(y_true=target, y_pred=pred))
        print("-----------------test_Confusion_matrix:-----------------")
        print(confusion_matrix(y_true=target, y_pred=pred))

        logger.info("seed:" + str(args.seed) + " test_F1:" + str(F1) + "  test_AUC:" + str(AUC))
